...
use super::Map;
use super::MapNode::{Leaf, Two, Three};
...
fn from_zipper<K, V>(ctx: List<TreeContext<K, V>>, tree: Map<K, V>) -> Map<K, V> {
    match ctx.uncons() {
        None => tree,
        Some((x, xs)) => {
            match x.deref().clone() {
...
pub fn ins_down<K: Ord, V>(ctx: List<TreeContext<K, V>>,
                           k: Arc<K>,
                           v: Arc<V>,
                           m: Map<K, V>)
                           -> Map<K, V> {
    match *m.0 {
        ...
        Two(_, ref left, ref k1, ref v1, ref right) => {
            match k.cmp(k1) {
                ...
                Ordering::Less => {
                    ins_down(ctx.cons(TwoLeft(k1.clone(), v1.clone(), right.clone())),
                             k,
                             v,
                             left.clone())
                }
                _ => {
                    ins_down(ctx.cons(TwoRight(left.clone(), k1.clone(), v1.clone())),
                             k,
                             v,
                             right.clone())
                }
            ...
            match k.cmp(k1) {
                ...
                c1 => {
                    match (c1, k.cmp(k2)) {
                        ...
                        (Ordering::Less, _) => {
                            ins_down(ctx.cons(ThreeLeft(k1.clone(),
                                                        v1.clone(),
                                                        mid.clone(),
                                                        k2.clone(),
                                                        v2.clone(),
                        ...
                        (Ordering::Greater, Ordering::Less) => {
                            ins_down(ctx.cons(ThreeMiddle(left.clone(),
                                                          k1.clone(),
                                                          v1.clone(),
                                                          k2.clone(),
                                                          v2.clone(),
                        ...
                        _ => {
                            ins_down(ctx.cons(ThreeRight(left.clone(),
                                                         k1.clone(),
                                                         v1.clone(),
                                                         mid.clone(),
                                                         k2.clone(),
...
fn ins_up<K, V>(ctx: List<TreeContext<K, V>>, kickup: KickUp<K, V>) -> Map<K, V> {
    match ctx.uncons() {
        None => {
            match kickup {
...
pub fn pop_down<K: Ord, V>(ctx: List<TreeContext<K, V>>,
                           k: &K,
                           m: Map<K, V>)
        ...
        Two(_, ref left, ref k1, ref v1, ref right) => {
            match (&*right.0, k.cmp(k1)) {
                ...
                (_, Ordering::Equal) => {
                    ...
                    Some((k1.clone(),
                          v1.clone(),
                          remove_max_node(ctx.cons(TwoLeft(max_key, max_val, right.clone())),
                                          left.clone())))
                }
                (_, Ordering::Less) => {
                    pop_down(ctx.cons(TwoLeft(k1.clone(), v1.clone(), right.clone())),
                             k,
                             left.clone())
                }
                (_, _) => {
                    pop_down(ctx.cons(TwoRight(left.clone(), k1.clone(), v1.clone())),
                             k,
                             right.clone())
                }
            }
        ...
        Three(_, ref left, ref k1, ref v1, ref mid, ref k2, ref v2, ref right) => {
            ...
            match (leaves, k.cmp(k1), k.cmp(k2)) {
                ...
                (_, Ordering::Equal, _) => {
                    ...
                    Some((k1.clone(),
                          v1.clone(),
                          remove_max_node(ctx.cons(ThreeLeft(max_key,
                                                             max_val,
                                                             mid.clone(),
                                                             k2.clone(),
                                                             v2.clone(),
                ...
                (_, _, Ordering::Equal) => {
                    ...
                    Some((k2.clone(),
                          v2.clone(),
                          remove_max_node(ctx.cons(ThreeMiddle(left.clone(),
                                                               k1.clone(),
                                                               v1.clone(),
                                                               max_key,
                                                               max_val,
                ...
                (_, Ordering::Less, _) => {
                    pop_down(ctx.cons(ThreeLeft(k1.clone(),
                                                v1.clone(),
                                                mid.clone(),
                                                k2.clone(),
                                                v2.clone(),
                ...
                (_, Ordering::Greater, Ordering::Less) => {
                    pop_down(ctx.cons(ThreeMiddle(left.clone(),
                                                  k1.clone(),
                                                  v1.clone(),
                                                  k2.clone(),
                                                  v2.clone(),
                ...
                _ => {
                    pop_down(ctx.cons(ThreeRight(left.clone(),
                                                 k1.clone(),
                                                 v1.clone(),
                                                 mid.clone(),
                                                 k2.clone(),
...
fn pop_up<K, V>(xs: List<TreeContext<K, V>>, tree: Map<K, V>) -> Map<K, V> {
    match xs.uncons() {
        None => tree,
        Some((x, ctx)) => {
